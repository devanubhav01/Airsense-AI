...
import { MapPin, TrendingUp, TrendingDown, CheckCircle2, Loader2 } from "lucide-react";
import {
...
import { POLLUTANT_LABELS, getBand } from "@/lib/data";

const CITY_NAMES = ["Delhi", "Mumbai", "Bengaluru", "Kolkata", "Chennai", "Greater Noida"];
...
function useCityData(cityName) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/city/${encodeURIComponent(cityName)}`)
      ...
      .then((json) => {
        if (cancelled) return;
        if (!json.success) throw new Error(json.error || "Failed to load city data");
        setData(json.data);
      })
      .catch((err) => {
...
export default function DashboardPage() {
  ...
  const { data: cityData, loading, error } = useCityData(city);
  const { data: compareData, loading: compareLoading } = useCityData(compareCity);
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center bg-slate-50 px-5">
        <div className="rounded-xl border border-red-200 bg-red-50 px-6 py-4 text-sm text-red-600">
          Couldn't load AQI data for {city}: {error}
        </div>
      </div>
    );
  }
  ...
  const forecast = cityData.forecast7Day && cityData.forecast7Day.length > 0 ? cityData.forecast7Day : [{ day: "Today", aqi: cityData.aqi }];
  const tomorrow = forecast[1]?.aqi ?? cityData.aqi;
  const trendUp = tomorrow > cityData.aqi;

  const pollutantData = Object.entries(POLLUTANT_LABELS).map(([key, label]) => ({
    name: label, value: cityData.pollutants?.[key] ?? 0,
  }));
    ...
    <div className="mx-auto max-w-7xl bg-slate-50 px-5 py-10 lg:px-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Last updated {cityData.lastFetchedAt ? new Date(cityData.lastFetchedAt).toLocaleTimeString() : "just now"}
          </p>
        </div>
      ...
      <div className="mt-7 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Card>
          ...
          <span className="mt-2 inline-block text-[11px] text-slate-500">{cityData.pollutants?.pm25 ?? "—"} µg/m³</span>
        </Card>
        <Card>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-3">
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-center">
              <select value={compareCity} onChange={(e) => setCompareCity(e.target.value)} className="mx-auto block bg-transparent text-xs text-slate-500 outline-none">
                {CITY_NAMES.filter((c) => c !== city).map((c) => <option key={c} value={c}>{c}</option>)}
              </select>
              <div className={`mt-1 text-3xl font-semibold ${compareData ? getBand(compareData.aqi).text : "text-slate-400"}`} style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                {compareLoading || !compareData ? "…" : compareData.aqi}
              </div>
            </div>
          </div>
        </Card>
        ...
        <Card className="border-indigo-200 bg-indigo-50/50">
          ...
          <Button variant="primary" className="mt-4 w-full" onClick={() => router.push(`/report?city=${city}`)}>Generate Report — ₹49</Button>
        </Card>
      </div>
    </div>
  );
