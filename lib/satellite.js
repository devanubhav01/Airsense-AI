 ...
 * few hours to process), so we try today first and fall back to the
 * previous 1-2 days before giving up.
 *
 * Returns { url, date, source } on success, or null if no usable image
 * could be found. Never throws — callers can safely `.catch()` around it,
 * but it's written to resolve instead so it can't crash a Promise.all.
 */
